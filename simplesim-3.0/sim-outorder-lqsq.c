...
//TODO add checkpoint recovery here
static void
ruu_writeback(void)
{
  int i;
	  ...
	  if ((MD_OP_FLAGS(op) & (F_MEM|F_LOAD)) == (F_MEM|F_LOAD)) 
	    {
	      ...
	      lq->op = op;
	      lq->PC = regs.regs_PC;
	      lq->next_PC = regs.regs_NPC; lsq->pred_PC = pred_PC;
	      lq->in_LSQ = TRUE;
	      lq->ea_comp = FALSE;
	      lq->recover_inst = FALSE;
	      lq->dir_update.pdir1 = lsq->dir_update.pdir2 = NULL;
...
//TODO: add checkpoints here based on confidence of branch prediction --> if it's a branch and the confidence is low, add a checkpoint to the checkpoint buffer
static void
ruu_fetch(void)
{
      ...
      if (pred)
	{
	  ...
	  if (MD_OP_FLAGS(op) & F_CTRL) {
	    ...
	    if (CONFIDENCE_LEVEL_LOW) {
	      ...
	      //TODOTODO fill in all the attributes
	      //new_checkpoint-> .....
